...
public class Grid< Thing> {
    ...
    public Grid( Grid< Thing > other ) {
        ...
        for ( int r = 0; r < this.nRows; ++r ) {
            if (other.spots[0][0].getClass().getSimpleName().equals("TipCoordinates")) {
                for (int c = 0; c < this.nCols; ++c) {
                    Thing classy = other.spots[r][c];
                    // this is a mess but it works cause cloneable interface sucks
                    TipCoordinates realClass = (TipCoordinates) classy;
                    this.spots[r][c] = (Thing) new TipCoordinates(realClass.row(), realClass.col(), realClass.getLetter());
                }
            } else {
